...
class RoutineManager: ObservableObject {
    ...
    init() {
        // Check model version before loading any data
        checkModelVersionAndResetIfNeeded()
        loadRoutines()
    ...
    func recordCompletedRoutine(_ routine: Routine, totalDuration: TimeInterval) {
        ...
        purgeOldCompletedRoutines()
        saveCompletedRoutines()
    }

    /// Removes completed routines older than the retention window.
    private func purgeOldCompletedRoutines() {
        let cutoff = Date().addingTimeInterval(-completedRoutineRetentionDays * 24 * 3600)
        let before = completedRoutines.count
        completedRoutines.removeAll { $0.completedAt < cutoff }
        if completedRoutines.count != before {
            print("Purged \(before - completedRoutines.count) completed routines older than \(Int(completedRoutineRetentionDays)) days")
        }
    }
    ...
    private func loadRoutines() {
        ...
        do {
            ...
            // Don't silently discard -- keep the raw data so we don't lose it
            // on a model mismatch. The version check above handles cleanup.
            routines = []
        }
    }
